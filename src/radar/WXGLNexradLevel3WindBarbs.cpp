...
vector<double> WXGLNexradLevel3WindBarbs::decodeAndPlot(const ProjectionNumbers& projectionNumbers, bool isGust, FileStorage& fileStorage) {
    ...
    for (const auto& windBarb : arrWb) {
        ...
        if (length > 4) {
            ...
            addAll(stormList, WXGLNexradLevel3Common::computeMercatorNumbersFromEc(ec, projectionNumbers));
            ...
            if (above50) {
                ...
                addAll(stormList, WXGLNexradLevel3Common::drawLine(ec, projectionNumbers, degree2 - arrowBend * 2.0, startLength + arrowLength * nmScaleFactor));
                // perpendicular line from main barb;
                ec = ExternalGeodeticCalculator::calculateEndingGlobalCoordinates(
                    end,
                    degree2,
                    barbOffset + startLength + -1.0 * arrowSpacing * nmScaleFactor * barbLengthScaleFactor);
                addAll(stormList, WXGLNexradLevel3Common::drawLine(ec, projectionNumbers, degree2 - 90.0, startLength + 0.80 * arrowLength * nmScaleFactor));
                // connecting line parallel to main barb;
                ec = ExternalGeodeticCalculator::calculateEndingGlobalCoordinates(
                    end,
                    degree2,
                    barbOffset + startLength + index * arrowSpacing * nmScaleFactor * barbLengthScaleFactor);
                addAll(stormList, WXGLNexradLevel3Common::drawLine(ec, projectionNumbers, degree2 - 180.0, startLength + 0.5 * arrowLength * nmScaleFactor));
                index += 1;
            }
            ...
            if (halfBarb) {
                ...
                addAll(stormList, WXGLNexradLevel3Common::drawLine(ec, projectionNumbers, degree2 - arrowBend * 2.0, startLength + arrowLength / 2.0 * nmScaleFactor));
            }
        }
    }
    return stormList;
