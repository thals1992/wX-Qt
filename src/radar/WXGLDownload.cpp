...
const string WXGLDownload::pattern1{">(sn.[0-9]{4})</a>"};
const string WXGLDownload::pattern2{".*?([0-9]{2}-[A-Za-z]{3}-[0-9]{4} [0-9]{2}:[0-9]{2}).*?"};
const string WXGLDownload::nwsRadarPub{"https://tgftp.nws.noaa.gov/"};
const vector<string> WXGLDownload::pacRids{"HKI", "HMO", "HKM", "HWA", "APD", "ACG", "AIH", "AHG", "AKC", "ABC", "AEC", "GUA"};

// Used by HI, STI, TVS
void WXGLDownload::getNidsTab(const string& product, const string& radarSite, FileStorage& fileStorage) {
...
string WXGLDownload::getRidPrefix(const string& radarSite, bool isTdwr) {
    if (isTdwr) {
        return "";
    } else {
        if (radarSite == "JUA") {
...
string WXGLDownload::getRadarDirectoryUrl(const string& radarSite, const string& product, const string& ridPrefix) {
...
string WXGLDownload::getRadarFileUrl(const string& radarSite, const string& product, bool isTdwr) {
...
void WXGLDownload::getRadarFilesForAnimation(QWidget * parent, int frameCount, const string& product, const string& radarSite, FileStorage * fileStorage) {
