...
const string UtilityMetar::metarFileName{"/us_metar3.txt"};
bool UtilityMetar::initializedObsMap{false};
unordered_map<string, LatLon> UtilityMetar::obsLatlon;
vector<string> UtilityMetar::metarDataRaw;
vector<RID> UtilityMetar::metarSites;
std::mutex UtilityMetar::mtx;

void UtilityMetar::getStateMetarArrayForWXOGL(const string& radarSite, FileStorage& fileStorage) {
...
string UtilityMetar::getObservationSites(const string& radarSite) {
    string obsListSb;
...
vector<string> UtilityMetar::condenseObs(const vector<string>& observations) {
...
RID UtilityMetar::findClosestObservation(const LatLon& latLon, int index) {
    readMetarData();
