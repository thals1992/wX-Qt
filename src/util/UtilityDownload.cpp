...
bool UtilityDownload::useNwsApi{false};

string UtilityDownload::getTextProduct(const string& produ) {
    ...
    if (prod == "AFDLOC") {
        ...
        text = getTextProduct("afd" + WString::toLower(tmp));
    } else if (prod == "HWOLOC") {
        const auto tmp = Location::office();
        text = getTextProduct("hwo" + WString::toLower(tmp));
...
string UtilityDownload::getImageProduct(const string& product) {
    string url;
    if (product == "STRPT") {
        ...
        const auto radarMosaicSector = UtilityNwsRadarMosaic::getNearestMosaic(Location::getLatLonCurrent());
        url = UtilityNwsRadarMosaic::get(radarMosaicSector);
    }
    return url;
}

string UtilityDownload::getTextProductWithVersion(const string& product, int version) {
...
string UtilityDownload::getRadarStatusMessage(const string& radarSite) {
    string ridSmall;
    if (radarSite.size() == 4) {
